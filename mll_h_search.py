...
class LocalModelTrainer():
    ...
    def h_search(self, logh_range, n_iter, lr, alpha):
        self.best_h = None
        min_train_mnmll = np.inf
        for logh in logh_range:
            ...
            try:
                ...
                continue
            model, mll, optimizer = self.get_model(local_idx, h, lr, alpha)
            model.to(self.device), mll.to(self.device)
            train_nmll, _, _ = self.optimize(n_iter=n_iter, model=model, mll=mll, optimizer=optimizer,
                                                                        val_local_idx=None)
            train_mnmll = train_nmll
            # print(np.exp(-train_nmll), len(local_idx), h)
            if train_mnmll < min_train_mnmll:
                min_train_mnmll = train_mnmll
                self.best_h = h
                effective_n_train_points = len(local_idx)
                self.best_model = copy.deepcopy(model)
        return min_train_mnmll, self.best_h, effective_n_train_points, 
...
if __name__ == '__main__':
    ...
    for dataset in ['boston']:
        ...
        for test_idx, x in tqdm(enumerate(x_test), total=len(x_test)):
            ...
            tmp_train_mnmll, best_h, effective_n_train_points =\
                lmtrainer.h_search(logh_range=np.arange(-5,2,0.5), n_iter=5, lr=0.01, alpha=0.1)
            train_mnmll += tmp_train_mnmll
        ...
        print(dataset)
        print('h list', list(map(lambda x: np.log(x), hs_list)))
